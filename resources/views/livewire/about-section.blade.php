        ...
        <div class="grid grid-cols-1 md:grid-cols-2 items-center gap-8 px-4 md:px-0">
            <div class="max-w-lg">
                ...
                @if ($article)
                    ...
                    <div class="mt-6">
                        <a href="{{ route('articles.show', $article->slug) }}" class="inline-block bg-black hover:bg-gray-700 text-white px-4 py-2 rounded">
                            Read more
                        </a>
                    </div>
                @else
            ...
            <div class="mt-12 md:mt-0 flex items-center justify-center">
                @if ($article && $article->primaryMedia)
                    ...
                    @elseif($m->type === 'youtube' && $m->youtube_id)
                        <iframe class="rounded-lg shadow-md w-full max-h-64" src="https://www.youtube.com/embed/{{ $m->youtube_id }}" title="YouTube video" frameborder="0" allowfullscreen></iframe>
                    @endif
                @else
                    <img src="{{ asset('aboutus/afunction.jpg') }}" alt="About image" class="object-cover rounded-lg shadow-md w-full" />
                @endif
            </div>
        </div>
    </div>
